...
function strTime(msec) {
    let answ = '**';
    let frags = new Date(msec).toUTCString().split(' ');
    if (frags[1]-1) {answ += `${frags[1]-1} д. `}
    let day = frags[4].split(':');
    if (day[0]-0) {answ += `${day[0]-0} ч. `}
    if (day[1]-0) {answ += `${day[1]-0} м. `}
    if (day[2]-0) {answ += `${day[2]-0} с. `}
    return answ+'**';
}
