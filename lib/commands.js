...
module.exports.rank = async function (id, nick) {
    try {
        if (!await db.existsAsync('user_'+id)) {
            ...
            let answ = {};
            answ.rank = rank;
            answ.text = `вы зарегистрировались и были поставлены в начало очереди на обновление. Ник: ${nick}, ранг ${rank}`
        } else {
            let answ = {};
            answ.text = 'ручное обновление больше не нужно! Теперь бот обновляет ранг автоматически с периодом '+strTime((await db.dbsizeAsync()-2)*60);
            answ.rank = -1;
            return answ;
        }
    } catch(err) {
...
module.exports.help = async function () {
    try {
        let answ = await fs.readFileAsync('./text/help.txt', 'utf8');
        return answ;
    } catch(err) {
        throw err;
    }
