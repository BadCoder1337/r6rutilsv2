...
module.exports.rank = async function (bot, msg, nick) {
    try {
        ...
        if (!exist & !!nick) {
            ...
            var answ = 'ручное обновление больше не нужно! Теперь бот обновляет ранг автоматически с периодом '+strTime((await db.dbsizeAsync()-2)*process.env.COOLDOWN/1000);
        } else if (!nick) {
