    ...
    try {
        ...
        guilds.forEach(async (e) => {
            try {
                ...
                let rId = rolesDB[ranks[rank].split(' ')[0]] || guildRoles.array()[0].id;
                await member.addRole(guildRoles.find('id', rId), '...готово!');
            } catch (err) {
                throw err;
            }
        });
...
module.exports.rank = async function (bot, msg, nick) {
    try {
        ...
        if (!exist & !!nick) {
            ...
            let rId = rolesDB[ranks[rank].split(' ')[0]];
            if (rId) {
                await member.addRole(guildRoles.find('id', rId ), 'пользователь зарегистрирован');
