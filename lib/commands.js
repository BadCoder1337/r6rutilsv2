...
module.exports.rank = async function (bot, msg, nick) {
    try {
        ...
        if (!exist & !!nick) {
            ...
            answ = 'вы уже зарегистрированы, бот обновляет ваш ранг автоматически с периодом ' + strTime(llen * (process.env.COOLDOWN)) + ' (' + llen + ' польз.)';
            if (nick) {
...
module.exports.unregister = async function (bot, msg, id) {
    try {
        ...
        return `пользователь ${user.tag} удален`;
    } catch (err) {
        throw err;
    }
}
