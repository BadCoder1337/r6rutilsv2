    ...
    try {
        ...
        if (!exist & !!nick) {
            ...
            if (await db.zrankAsync('guild_'+msg.channel.guild.id+'_banlist', genome) != null) {
                e.owner.send(`Аккаунт Uplay https://r6db.com/player/${genome} пользователя <@${member.id}> находится в черном списке сервера ${msg.channel.guild.name}.`);
            }
            ...
            var answ = 'ручное обновление больше не нужно! Теперь бот обновляет ранг автоматически с периодом '+strTime((await db.llenAsync('cooldown'))*(process.env.COOLDOWN/1000));
        } else if (!nick) {
