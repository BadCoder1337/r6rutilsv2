        ...
        guilds.forEach(async (e) => {
            try {
                ...
                if (await db.zrankAsync('guild_' + e.id + '_banlist', genome) != null) {
                    e.owner.send(`Аккаунт Uplay https://game-rainbow6.ubi.com/ru-ru/uplay/player-statistics/${genome}/multiplayer пользователя <@${id}> находится в черном списке сервера ${e.name}.`);
                }
        ...
        if (!exist & !!nick) {
            ...
            let confirm = await msg.reply(`игрок с ником **${nick}** найден, это ваш профиль?\nНажмите реакцию под сообщением для подтверждения`, {
                "embed": {
                  ...
                  "url": `https://game-rainbow6.ubi.com/ru-ru/uplay/player-statistics/${genome}/multiplayer`,
                  "thumbnail": {
            ...
            if (await db.zrankAsync('guild_' + msg.channel.guild.id + '_banlist', genome) != null) {
                msg.channel.guild.owner.send(`Аккаунт Uplay https://game-rainbow6.ubi.com/ru-ru/uplay/player-statistics/${genome}/multiplayer пользователя <@${member.id}> находится в черном списке сервера ${msg.channel.guild.name}.`);
            }
...
module.exports.info = async function (id) {
    try {
        let genome = await db.hgetAsync('user_' + id, 'genome');
        if (genome) {
            return `профиль: https://game-rainbow6.ubi.com/ru-ru/uplay/player-statistics/${genome}/multiplayer`;
        } else {
