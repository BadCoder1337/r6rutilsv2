...
module.exports.rank = async function (id, nick) {
    try {
        if (!await db.existsAsync('user_'+id)) {
            ...
            let answ = `вы зарегистрировались и были поставлены в начало очереди на обновление. Ник: ${nick}, ранг ${rank}`
        } else {
            let answ = 'ручное обновление больше не нужно! Теперь бот обновляет ранг автоматически с периодом '+strTime((await db.dbsizeAsync()-2)*60);
            return answ;
        }
    } catch(err) {
        throw err;
