...
function strTime(msec) {
    ...
    if (date.getUTCDate()-1) {answ += `${date.getUTCDate()-1} д. `}
    if (date.getUTCHours()) {answ += `${date.getUTCHours()} ч. `}
    if (date.getUTCMinutes()) {answ += `${date.getUTCMinutes()} м. `}
    if (date.getUTCSeconds()) {answ += `${date.getUTCSeconds()} с. `}
    return answ+'**';
}
...
module.exports.rank = async function (bot, msg, nick) {
    try {
        ...
        if (!exist & !!nick) {
            ...
            var answ = 'вы уже зарегистрированы, бот обновляет ваш ранг автоматически с периодом '+strTime(llen*(process.env.COOLDOWN))+' ('+llen+' польз.)';
        } else if (!nick) {
...
module.exports.info = async function (id) {
    try {
        ...
        if (genome) {
            ...
            return 'пользователь не зарегистрирован или вы его не упомянули'
        }
    } catch (err) {
        throw err;
    }
...
module.exports.stats = async function (g) {
    try {
        ...
        Object.keys(rolesDB).forEach(e => {
            if (e == 'diamondStab') {return;}
            let r = g.roles.get(rolesDB[e]);
            roles.push(r);
        })
        roles.forEach((e, i) => {
            if (e.name == '@everyone') {return;}
