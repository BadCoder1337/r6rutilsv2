...
module.exports.rank = async function (bot, msg, nick) {
    try {
        ...
        if (!exist & !!nick) {
            ...
            let stats = await r6db.getStats(genome);
            let confirm = await msg.reply(`игрок с ником **${nick}** найден, это ваш профиль?\nНажмите реакцию под сообщением для подтверждения`, {
                "embed": {
                  ...
                  "url": `https://r6db.com/player/${genome}`,
                  "thumbnail": {
                  ...
                  "fields": [
                    {
                      ...
                      "value": `**В:** ${stats[0].matchesWon} **П:** ${stats[0].matchesLost}\n**В%:** ${(100*stats[0].matchesWon/(stats[0].matchesWon+stats[0].matchesLost)).toFixed(2)}%`,
                      "inline": true
                    },
                    {
                      "name": "Убийства/Смерти",
                      "value": `**У:** ${stats[0].kills} **С:** ${stats[0].deaths}\n**У/С:** ${(stats[0].kills/(stats[0].deaths || 1)).toFixed(2)}`,
                      "inline": true
                    }
                  ]
                }
