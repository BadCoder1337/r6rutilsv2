        ...
        guilds.forEach(async (e) => {
            try {
                ...
                if (await db.zrankAsync('guild_' + e.id + '_banlist', genome) != null) {
                    e.owner.send(`Аккаунт Uplay https://r6tab.com/${genome} пользователя <@${id}> находится в черном списке сервера ${e.name}.`);
                }
...
module.exports.countTwinks = async (channel, bot) => {
    try {
        ...
        const userGens = await Promise.all(dbRequests);
        let answ = Object.entries(userPool.map((e, i) => ({count: 1, genome: userGens[i], id: e})).reduce((a, b) => {
        ...
        //     return `пользователь с Uplay \`${nick}\` в Discord не найден`
        // }
        await channel.send((await Promise.all(answ.map(async (i) =>`Uplay <https://r6tab.com/${i[1]}> привязан:\n    ${(await Promise.all(i[0].map(async (id) => {try {
            return `<@${id}> \`${(await bot.fetchUser(id)).tag}\``
        } catch (err) {
            console.log(id, err)
        }}))).join('\n    ')}`))).join('\n•'), {split: true});
        // (await Promise.all(answ.map(async (i) => `Uplay https://r6tab.com/${i[1]} привязан:\n`+(await Promise.all(i[0].map(async (id) => `<@${id}> ${await bot.fetchUser(id).tag}`)))))).join('\n    ')).join('\n•')
