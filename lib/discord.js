...
async function interval() {
    try {
        ...
        pack.ids.forEach(async (e, i) => {
            let commonGuilds = bot.guilds.filterArray(e => {
                let m = e.member(bot.users.get(e));
                return !!m;
            });
            ...
            await cmds.update(bot, commonGuilds, e, pack.genomes[i], ranks[i]);
        });
    } catch (err) {
...
bot.on('message', async (message) => {
    try {
        ...
        if (message.content.startsWith(prefix) & message.channel.type == 'text' & !waitingList.includes(message.author.id)) {
            ...
            let msg = message.content.replace(/ +(?= )/g, ''); //убираем множественные пробелы
            ...
            switch (command[0].toLowerCase()) {
                ...
                case prefix + 'cooldown':
                    if (message.author.id == process.env.SUPPORT_ID) {
                        clearInterval(timer);
                        setInterval(interval, parseInt(command[1]) || process.env.COOLDOWN);
                    } else {
